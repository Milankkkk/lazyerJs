var lazyer_datePicker = {
    ...
    init:function () {
        let _this = this;
        let $ = lazyerJs.$;
        var dateHtml="<div id='dateInputBox'><i class='fa fa-calendar'></i><i class='fa fa-times-circle-o' onclick='clearData()'></i><input type='text' id='dateInput' placeholder='选择日期'></div>" +
        ...
        $('#dateInput').on('input',function () {
            if($('#dateInput').val().split('-')[0] && $('#dateInput').val().split('-')[1]){
                var yVal = $('#dateInput').val().split('-')[0].replace(/\b(0+)/gi,"")
                var mVal = $('#dateInput').val().split('-')[1].replace(/\b(0+)/gi,"")
